...
When an Android device disconnects from PhoneBook, the device's USB port state changes leading to `adbd` restart. Android's `init` process kills all the service child processes on `stop service` command (issued for `adbd` here). This makes impossible graceful applet shutdown with restoring the device to the original state. The current workaround is exactly to use `su`: the `init` has no permissions to kill the applet launched under `su`.
